...
export const BookingForm = (state) => {
   ...
   return <div>
      <form action={state.submitForm({ time: time, date: date, guests: guests })}>
         ...
         <input type="date" required id="res-date" value={date} onChange={e => {setDate(e.target.value); availableTimesUpdate(e)}} min={`${new Date().toLocaleString("default", { year: 'numeric' })}-${new Date().toLocaleString("default", { month: '2-digit' })}-${new Date().toLocaleDateString("default", { day: '2-digit' })}`}></input>
